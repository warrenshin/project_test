...
CSV_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "주식상황_new.csv")
...
def download_and_load():
    try:
        ...
        if not os.path.exists(CSV_PATH):
            st.error("데이터를 다운로드할 수 없고 저장된 파일도 없습니다.")
            st.stop()
...
def _fetch_article_text(url, timeout=6):
    try:
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
        resp = requests.get(url, timeout=timeout, headers=headers)
        resp.raise_for_status()
        soup = BeautifulSoup(resp.text, "html.parser")
        for sel in ["article", ".article-body", ".entry-content", ".post-content", "main"]:
            elems = soup.select(sel)
            if elems:
                text = " ".join(e.get_text(separator=" ", strip=True) for e in elems[:3])
...
def fetch_fj_detailed():
    ...
    try:
        ...
        for entry in feed.entries[:12]:
            ...
            if title:
                items.append((title, content, link, pub))
        return items
    except Exception as e:
        return [(f"뉴스 불러오기 실패: {e}", "", "", "")]
...
@st.cache_data(ttl=600)
def fetch_tipranks_news():
    try:
        headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"}
        resp = requests.get("https://www.tipranks.com/news", timeout=8, headers=headers)
        resp.raise_for_status()
        soup = BeautifulSoup(resp.text, "html.parser")
        items = []
        for a in soup.find_all("a", href=True):
            ...
            if len(title) < 20:
                continue
            full_url = href if href.startswith("http") else f"https://www.tipranks.com{href}"
            if any(x[2] == full_url for x in items):
                continue
            ...
            if parent:
                ...
                summary = summary[:400]
            items.append((title, summary, full_url, ""))
            if len(items) >= 12:
                break
        return items if items else [("기사를 불러올 수 없습니다.", "", "", "")]
    except Exception as e:
        return [(f"TipRanks 뉴스 불러오기 실패: {e}", "", "", "")]
...
# ─── 탭: 차트 / 뉴스 ────────────────────────────────────────
tab_chart, tab_yahoo, tab_cnbc, tab_fj, tab_tr = st.tabs(["🥧 비중 차트", "📰 Yahoo 뉴스", "🌐 CNBC 뉴스", "⚡ Financial Juice", "📊 TipRanks"])
...
with tab_fj:
    if st.button("Financial Juice 뉴스 불러오기", key="fj_btn"):
        with st.spinner("Financial Juice 뉴스 및 기사 내용 로딩 중..."):
            news = fetch_fj_detailed()
            for i, (title, content, link, pub) in enumerate(news, 1):
                korean_title = translate(title)
                time_str = f" `{pub}`" if pub else ""
                st.markdown(f"**{i}. {korean_title}**{time_str}")
                if content:
                    korean_content = translate(content[:600]) if len(content) > 10 else ""
                    if korean_content:
                        with st.expander("📄 기사 내용 요약 보기"):
                            st.write(korean_content)
                if link:
                    st.markdown(f"🔗 [{link}]({link})")
                st.divider()

with tab_tr:
    if st.button("TipRanks 뉴스 불러오기", key="tr_btn"):
        with st.spinner("TipRanks 뉴스 로딩 중..."):
            news = fetch_tipranks_news()
            for i, (title, summary, link, pub) in enumerate(news, 1):
                korean_title = translate(title)
                st.markdown(f"**{i}. {korean_title}**")
                if summary:
