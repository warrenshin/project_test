...
def test_market_buy_cash_invariant_matches_preview(db):
    ...
    preview = client.post(
        f"/v1/portfolios/{portfolio_id}/orders/preview", json=order_payload, cookies=cookies
    ).json()
    assert preview["estimated_fillable"] is True

    cash_before = Decimal(
        str(client.get(f"/v1/portfolios/{portfolio_id}", cookies=cookies).json()["cash_balance"])
    )

    order_res = client.post(
        f"/v1/portfolios/{portfolio_id}/orders", json=order_payload, cookies=cookies, headers=_idem_headers()
    )
...
def test_market_sell_increases_cash_and_realizes_pnl(db):
    ...
    buy_res = client.post(
        f"/v1/portfolios/{portfolio_id}/orders", json=buy_payload, cookies=cookies, headers=_idem_headers()
    )
    ...
    preview = client.post(
        f"/v1/portfolios/{portfolio_id}/orders/preview", json=sell_payload, cookies=cookies
    ).json()

    sell_res = client.post(
        f"/v1/portfolios/{portfolio_id}/orders", json=sell_payload, cookies=cookies, headers=_idem_headers()
    )
...
def test_duplicate_idempotency_key_does_not_double_execute(db):
    ...
    first = client.post(
        f"/v1/portfolios/{portfolio_id}/orders", json=order_payload, cookies=cookies, headers=idem_headers
    )
    assert first.status_code == 201
    first_order_id = first.json()["id"]

    cash_after_first = client.get(f"/v1/portfolios/{portfolio_id}", cookies=cookies).json()["cash_balance"]
    ...
    cash_after_second = client.get(f"/v1/portfolios/{portfolio_id}", cookies=cookies).json()["cash_balance"]
...
def test_limit_order_not_immediately_fillable_stays_accepted_then_cancellable(db):
    ...
    cash_before = client.get(f"/v1/portfolios/{portfolio_id}", cookies=cookies).json()["cash_balance"]
    ...
    cash_after = client.get(f"/v1/portfolios/{portfolio_id}", cookies=cookies).json()["cash_balance"]
    assert cash_before == cash_after  # 미체결 상태에서는 현금이 움직이지 않는다

    cancel_res = client.post(f"/v1/orders/{order['id']}/cancel", cookies=cookies)
    assert cancel_res.status_code == 200
    assert cancel_res.json()["status"] == "CANCELLED"

    # 종료 상태 주문은 다시 취소할 수 없다
    second_cancel = client.post(f"/v1/orders/{order['id']}/cancel", cookies=cookies)
