...
### 게시된 콘텐츠 migration 롤백 주의사항

`scripts/publish_lesson.py`로 강의(예: 6~15강)를 승인·게시하면 `lesson_review_audits`에
누가·언제·어떤 근거로 승인했는지 감사기록이 남는다. **감사기록이 하나라도 존재하는
콘텐츠는 그 콘텐츠를 시드한 migration을 자동으로 downgrade할 수 없다** — 해당
migration의 `downgrade()`는 삭제를 시작하기 전에 감사기록 존재 여부부터 확인하고,
있으면 아무 것도 바꾸지 않고 `RuntimeError`로 즉시 중단한다(외래키 위반으로
도중에 실패해 절반만 지워지는 상태를 막기 위함). 감사기록은 절대 자동으로
삭제하지 않는다 — 승인 이력을 조용히 지우는 대신, 그 판단은 항상 운영자가
내리도록 설계했다.

이것이 의미하는 것:

- **코드 롤백과 DB 콘텐츠 롤백은 별개로 판단해야 한다.** 애플리케이션 코드(이미지)를
  이전 버전으로 되돌리는 것과, DB에 시드된 강의 콘텐츠를 이전 상태로 되돌리는 것은
  서로 다른 결정이다 — 코드만 롤백하고 DB는 그대로 두는 것이 대부분의 경우 더
  안전하다(이미 게시된 콘텐츠를 사용자가 보고 있었다면 특히 그렇다).
- 콘텐츠 자체를 되돌려야 하는 예외적인 상황이라면, 먼저 감사기록을 보존할지
  여부(예: 별도 테이블/백업으로 옮겨두고 진행)를 사람이 판단한 뒤 별도 절차를
  수립해야 한다. `alembic downgrade`를 그대로 재시도해서 되는 문제가 아니다.
- **운영 DB에 최초로 강의를 게시하기 전에는 반드시 DB 백업을 만들어 둔다.** 게시
  이후에는 위 제약 때문에 migration 되돌리기가 더 이상 간단하지 않으므로, 백업이
  유일한 안전망이 된다.
